...
class ReactFlagsSelect extends Component {
  ...
  filterSearch(evt) {
    ...
    const filteredCountries = filterValue && this.state.countries.filter((key) => {
  ...
  setCountries() {
    ...
    let selectCountries = this.props.countries && this.props.countries.filter(country => countries[country]);

    // Filter BlackList
    if (this.props.blackList && selectCountries) {
      selectCountries = fullCountries.filter(countryKey => selectCountries.filter(country => countryKey === country).length === 0);
    }
  ...
  render() {
    ...
    const optionsSize = this.props.optionsSize;
